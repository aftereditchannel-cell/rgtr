...
if (empty.settings.cloud.provider !== 'firebase') throw new Error('Firebase must remain the default')
if (!empty.settings.cloud.providerState?.firebase || !empty.settings.cloud.providerState?.googleDrive) throw new Error('missing separate sync states')
const driveRoundTrip = migrate({ ...back, version: 5, settings: { ...back.settings, cloud: { ...back.settings.cloud, provider: 'googleDrive', googleScriptUrl: 'https://script.google.com/macros/s/test/exec' } } })
if (driveRoundTrip.settings.cloud.provider !== 'googleDrive' || driveRoundTrip.settings.cloud.autoPull) throw new Error('Drive provider/manual pull migration failed')
console.log('cloud providers: Firebase default + Google Drive optional/manual refresh')
