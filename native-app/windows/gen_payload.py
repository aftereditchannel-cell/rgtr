...
APP = os.path.join(HERE, 'NEXUS-HQ-NativeCore.exe')
APP32 = os.path.join(HERE, 'NEXUS-HQ-NativeCore-x86.exe')
ICO = os.path.normpath(os.path.join(HERE, '..', '..', 'electron', 'icons', 'icon.ico'))
OUT = os.path.join(HERE, 'payload.cpp')
...
'''
with open(os.path.join(HERE, 'payload.hpp'), 'w') as f:
    f.write(_hpp if False else hpp)
print('payload.cpp/hpp written,', os.path.getsize(OUT) // 1024, 'KB')
