...
#define APP_NAME_W L"NEXUS HQ Native Core"
#define APP_VERSION_W L"1.0.0"
#define APP_EXE_W L"NEXUS-HQ-NativeCore.exe"
#define UNINS_W L"unins000.exe"
#define REG_UNINST L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\NEXUS HQ Native Core"
...
static int do_install() {
  ...
  swprintf(g_dir, MAX_PATH, L"%s\\NEXUS HQ Native Core", local);
  ...
  // انتخاب معماری درست سیستم (مثل نصب‌کننده‌های حرفه‌ای)
  BOOL wow64 = FALSE;
  IsWow64Process(GetCurrentProcess(), &wow64);
  const unsigned char* exe_data = wow64 ? PAYLOAD[0].data : PAYLOAD[1].data;
  size_t exe_size = wow64 ? PAYLOAD[0].size : PAYLOAD[1].size;
  ...
  DWORD wr = 0;
  WriteFile(f, exe_data, (DWORD)exe_size, &wr, NULL);
  CloseHandle(f);

  f = CreateFileW(ico_path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  if (f != INVALID_HANDLE_VALUE) {
    WriteFile(f, PAYLOAD[2].data, (DWORD)PAYLOAD[2].size, &wr, NULL);
    CloseHandle(f);
  }
  ...
  swprintf(start_dir, MAX_PATH, L"%s\\NEXUS HQ Native Core", programs);
  ...
  if (RegCreateKeyExW(HKEY_CURRENT_USER, REG_UNINST, 0, NULL, 0, KEY_WRITE, NULL, &k, NULL) ==
    ...
    reg_set(k, L"Publisher", L"NEXUS HQ Native Core");
...
static int do_uninstall() {
  ...
  delete_file_if(lnk);
  swprintf(lnk, MAX_PATH, L"%s\\NEXUS HQ Native Core", programs);
