...
| قفل برنامه | PIN + **باز کردن با اثر انگشت** (Settings ← امنیت و قفل) |
...
نیازمندی‌ها: **JDK 21+** و **Android SDK** (پلتفرم ۳۵ + build-tools ۳۴ و ۳۵).
...
### ذخیره‌ی خودکار روی گیت
بعد از اتصال، گزینه‌ی **«ذخیره‌ی خودکار روی گیت»** (به‌صورت پیش‌فرض روشن) فعال می‌شود:
- هر تغییری که می‌دهید، چند ثانیه بعد بی‌صدا روی Gist نوشته می‌شود.
- هنگام باز کردن برنامه، آخرین نسخه‌ی ابری بررسی می‌شود و اگر جدیدتر باشد، همین دستگاه با آن به‌روز می‌شود.

یعنی روی هر دستگاه/اکانتی که همان توکن و شناسه را وارد کنید، تغییرات‌تان خودکار برمی‌گردد.
