...
| منو | نوار پایین (داشبورد/تصمیم/تحلیل/تنظیمات/بیشتر) + دکمه‌ی ☰ |
| میان‌برهای کیبورد | کاربرد ندارد — بقیه‌ی امکانات کامل است |
| قفل | رمز ۴ تا ۶ رقمی + **اثر انگشت** (در گوشی‌های دارای سنسور) + قفل خودکار |
...
کلید امضا (`android/keystore/nexus-hq.jks`) و رمزهایش **در گیت نیستند** (در `.gitignore`).
برای ساخت نسخه‌ی امضاشده:

1. اگر کلید را دارید: در پوشه‌ی `android/` فایل `keystore.properties.example` را به
   `keystore.properties` کپی کنید و رمزهای خودتان را در آن بنویسید
   (یا به‌جایش متغیرهای محیطی `NEXUS_STORE_PASSWORD` و `NEXUS_KEY_PASSWORD`).
2. اگر کلید را گم کرده‌اید، از تاریخچه‌ی گیت بازیابی کنید:
   `git show HEAD:android/keystore/nexus-hq.jks > android/keystore/nexus-hq.jks`
   (رمزها همان‌هایی هستند که موقع ساخت کلید زدید؛ اگر فراموش کرده‌اید،
   کلید جدید بسازید — ولی در این صورت به‌روزرسانی روی نسخه‌ی نصب‌شده‌ی قبلی ممکن نیست).
3. بدون کلید هم `assembleRelease` کار می‌کند، فقط خروجی امضانشده است.

**مشخصات:** شناسه `app.nexushq.mobile` · حداقل اندروید ۷ (API 24) · هدف API 35 · دسترسی فقط اینترنت.
...
| `npm run lock:check` | تست قفل برنامه (هش رمز، تلاش‌ها، صفحه‌ی قفل) — بدون نمایشگر |
...
| پوسته | **تیره / روشن / همراه سیستم** — در حالت «همراه سیستم» با ویندوز یا اندروید عوض می‌شود |
...
**تنظیمات ← قفل و امنیت** — برنامه را با یک رمز ۴ تا ۶ رقمی قفل کنید:

- **رمز عبور** — فقط **هش PBKDF2** (SHA-256 · ۲۱۰٬۰۰۰ تکرار · نمک تصادفی) روی دستگاه می‌ماند؛
  رمز به‌صورت متن ساده هیچ‌جا ذخیره نمی‌شود و در بکاپ/ابر هم نمی‌رود.
- **قفل خودکار** — همیشه · بعد از ۱/۵/۱۵/۳۰/۶۰ دقیقه بی‌کاری · یا فقط هنگام باز شدن برنامه.
  در اندروید، برگشتن از پس‌زمینه هم قفل را بررسی می‌کند.
- **اثر انگشت (اندروید)** — در گوشی‌های دارای سنسور، قفل با اثر انگشت هم باز می‌شود
  (فقط میان‌بر است؛ رمز همیشه کار می‌کند).
- **۵ تلاش اشتباه = ۳۰ ثانیه قفل موقت**، با شمارش معکوس روی صفحه‌ی قفل.
- **ویندوز:** میان‌بر `Ctrl+L` یا منوی «پرونده ← قفل فوری» برای قفل‌کردن دستی.
- صفحه‌ی قفل کاملاً **شیشه‌ای** است و با پوسته (تیره/روشن/همراه سیستم) هماهنگ می‌شود.

> رمز فقط جلوی بازکردن برنامه را می‌گیرد؛ فایل داده‌ی روی دیسک رمزنگاری‌شده نیست.
> اگر رمز را فراموش کردید، با پاک‌کردن `localStorage` (مرورگر) یا پوشه‌ی `data`
> (ویندوز/اندروید) قفل باز می‌شود — داده‌ی خودتان محفوظ است.
