...
## ✨ تازه‌های این نسخه (v1.2.0 — نسخه‌ی نهایی «final»)

| قابلیت | کجاست |
|---|---|
| 🌐 **مرکز شبکه‌های اجتماعی** | منوی کناری ← «شبکه‌های اجتماعی» — اینستاگرام/یوتیوب/تلگرام/ساندکلاد/اسپاتیفای/لینک‌های عمومی، رفرش خودکار، کلیدهای اختیاری |
| 🤖 **هوش مصنوعی و اتوماسیون** | منوی کناری ← «هوش مصنوعی» — چت زنده با Groq/OpenRouter/Gemini/Together/هر سرویس OpenAI-compatible + تسک‌های قالبی + جریان‌های کاری (Fetch → AI → Report) |
| 🔐 **قفل + اثر انگشت/چهره** | تنظیمات ← قفل و امنیت — رمز ۴ تا ۶ رقمی (PBKDF2)، ۵ تلاش و قفل موقت ۳۰ ثانیه‌ای، یادآور رمز، قفل فوری با Ctrl+L (ویندوز) |
| 🎨 **پوسته تیره/روشن/سیستم + شیشه‌ای** | تنظیمات ← زبان و نمایش ← پوسته + «افکت شیشه‌ای» |
| ☁️ **همگام‌سازی خودکار (Gist)** | تنظیمات ← همگام‌سازی ابری ← «ذخیره خودکار بعد از هر تغییر» و «دریافت خودکار» + ذخیره روی ابر هنگام بستن/مخفی‌شدن صفحه |
| 🧑‍🎨 **برند شخصی** | تنظیمات ← برند و هویت — نام برنامه، لوگوی سایدبار، آیکون اپ (+ اسکریپت `apply-branding`) |
| ⬆️ **بروزرسانی خودکار** | تنظیمات ← بروزرسانی برنامه — بررسی GitHub Releases، دانلود با نوار پیشرفت، اجرای نصب‌کننده |
| 📈 **تشخیص خودکار فالوور/بیو** | در فرم رسانه/شبکه‌های اجتماعی دکمه‌ی «پر کردن خودکار» + تازه‌سازی خودکار در صفحه‌ی ماژول |
| 🧭 **آموزش و تور معرفی** | منوی کناری ← «راهنما» — تور قدم‌به‌قدم + نقشه‌ی ویرایش روی گیت‌هاب |
| 📐 **منوهای کشویی داخل برنامه** | رفع منوی تمام‌صفحه‌ی اندروید — همه‌جا از `Dropdown` سفارشی استفاده می‌شود |
| 🏢 **ساخت دپارتمان از قالب** | تنظیمات ← دپارتمان‌ها ← «از قالب بسازید» |
| 🖥️ **نسخه‌ی بومی (Native)** | `native-app/` — اپ ویندوز Win32 و اندروید smali بدون مرورگر (منبع در `native-app/`، اسکریپت‌های ساخت در `build-*`) |
| 🎯 **لوگوی موج زرد** | روی ویندوز، اندروید و وب — `scripts/make-icons.py` آن را بازتولید می‌کند |

> **همگام‌سازی خودکار:** توکن گیت‌هاب فقط روی خود دستگاه می‌ماند. با روشن بودن
> «ذخیره خودکار» و «دریافت خودکار»، هر تغییری که روی گوشی بدهید چند ثانیه بعد روی
> گیت نوشته می‌شود و دستگاه دیگر هنگام باز شدن خودکار آن را می‌گیرد (آخرین تغییر برنده است).

---

## 🪟 نسخه‌ی ویندوز (پیشنهادی)

### ⬆️ بروزرسانی خودکار (از v1.1.0)
تنظیمات ← **بروزرسانی برنامه**: برنامه خودش GitHub Releases را بررسی می‌کند،
نسخه‌ی جدید را **با نوار پیشرفت دانلود می‌کند** و با یک کلیک نصب‌کننده را اجرا می‌کند
(برنامه بسته می‌شود و نصب شروع می‌شود). امکان انتخاب **هر نسخه‌ای** از لیست و دانلود
خودکار آن هم هست. کل دانلود در فرآیند اصلی برنامه انجام می‌شود — بدون CORS و بدون
خطای «اتصال برقرار نشد»؛ اگر شبکه قطع باشد ۳ بار خودش تلاش می‌کند و در صورت نیاز از
مسیر جایگزین (ریدایرکت releases/latest) می‌خواند.
