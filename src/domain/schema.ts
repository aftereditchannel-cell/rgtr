...
export const CORE_MODULES: ModuleDef[] = [
  ...
  {
    key: 'planner', label: 'My Planner', labelFa: 'برنامه‌ریزی شخصی', icon: 'CalendarDays', group: 'core',
    desc: 'برنامه‌ریزی روزانه، هفتگی و جلسات شخصی با اتصال به سایر بخش‌ها',
    titleField: 'title', defaultView: 'calendar', views: ['calendar', 'kanban', 'table'], groupBy: 'status', dateField: 'date',
    fields: [
      f('title', 'Task / Event', 'text', { col: true }),
      f('kind', 'Type', 'select', { col: true, options: ['Meeting', 'Call', 'Focus Work', 'Event', 'Reminder'] }),
      f('status', 'Status', 'select', { col: true, options: ['To Do', 'In Progress', 'Done', 'Cancelled'] }),
      f('date', 'Date', 'date', { col: true }),
      f('time', 'Time', 'text', { placeholder: 'e.g. 14:30', col: true }),
      f('priority', 'Priority', 'select', { col: true, options: PRIORITY }),
      f('project', 'Related Project', 'ref', { refModule: 'projects' }),
      f('artist', 'Related Artist', 'ref', { refModule: 'artists' }),
      f('client', 'Related Client', 'ref', { refModule: 'clients' }),
      f('team', 'Related Person', 'ref', { refModule: 'team' }),
      f('location', 'Location / Link', 'url'),
      f('checklist', 'Checklist', 'checklist'),
      f('notes', 'Notes', 'textarea'),
    ],
  },
  ...
  /* ---------- MEDIA ---------- */
  {
    ...
    fields: [
      ...
      f('phone', 'Phone', 'text'),
      f('email', 'Email', 'text'),
      f('credentials', 'Credentials', 'textarea'),
      f('notes', 'Notes', 'textarea'),
    ],
  },
  {
    ...
    fields: [
      ...
      f('spotify', 'Spotify', 'url'),
      f('soundcloud', 'SoundCloud', 'url'),
      f('phone', 'Phone', 'text'),
      f('email', 'Email', 'text'),
      f('credentials', 'Credentials', 'textarea', { help: 'Passwords / Access info' }),
      f('personalInfo', 'Personal Info', 'textarea'),
  ...
  {
    key: 'labels', label: 'Labels', labelFa: 'لیبل‌ها', icon: 'Building2', group: 'media',
    desc: 'لیبل‌های موسیقی و شرکت‌های پخش',
    titleField: 'name', defaultView: 'cards', views: ['cards', 'table'],
    fields: [
      f('name', 'Label Name', 'text', { col: true }),
      f('website', 'Website', 'url', { col: true }),
      ...
      f('credentials', 'Credentials', 'textarea'),
      f('status', 'Status', 'select', { col: true, options: ['Active', 'Prospect', 'Partner'] }),
      f('notes', 'Notes', 'textarea'),
    ],
  },
  {
