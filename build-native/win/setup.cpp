...
#define APP_NAME_W L"NEXUS HQ"
#define APP_VERSION_W L"1.0.0"
#define APP_EXE_W L"NEXUS-HQ.exe"
#define UNINS_W L"unins000.exe"
#define REG_UNINST L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\NEXUS HQ"
...
static int do_install() {
  ...
  swprintf(g_dir, MAX_PATH, L"%s\\NEXUS HQ", local);
  ...
  swprintf(start_dir, MAX_PATH, L"%s\\NEXUS HQ", programs);
  ...
  if (RegCreateKeyExW(HKEY_CURRENT_USER, REG_UNINST, 0, NULL, 0, KEY_WRITE, NULL, &k, NULL) ==
    ...
    reg_set(k, L"Publisher", L"NEXUS HQ");
...
static int do_uninstall() {
  ...
  delete_file_if(lnk);
  swprintf(lnk, MAX_PATH, L"%s\\NEXUS HQ", programs);
